...
if (!lines.some((line) => line.trimEnd() === "openapi: 3.1.0")) {
  errors.push("OpenAPI version must be 3.1.0.");
}

const infoIndex = lines.findIndex((line) => line.trimEnd() === "info:");
let hasFrozenVersion = false;
if (infoIndex >= 0) {
  for (let i = infoIndex + 1; i < lines.length; i += 1) {
    ...
    if (line.trimEnd() === "  version: 1.0.0") {
      hasFrozenVersion = true;
      break;
    }
  }
}
